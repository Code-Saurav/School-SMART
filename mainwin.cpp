...
Mainwin::Mainwin() :comboboxtext{Gtk::manage(new Gtk::ComboBoxText{true})} {
    ...
    Gtk::Image* new_image = Gtk::manage(new Gtk::Image{"newSchool.png"});
    ...
    Gtk::Image* open_image = Gtk::manage(new Gtk::Image{"open_icon.png"});
    ...
    Gtk::Image* save_image = Gtk::manage(new Gtk::Image{"save_icon.png"});
    ...
    Gtk::Image* save_as_image = Gtk::manage(new Gtk::Image{"save_as_icon.png"});
   ...
   // Add a little space 
    ...
    Gtk::Image* student_image = Gtk::manage(new Gtk::Image{"button_student.png"});
    ...
    Gtk::Image* parent_image = Gtk::manage(new Gtk::Image{"button_parent.png"});
    ...
    Gtk::Image* ps_relate_image = Gtk::manage(new Gtk::Image{"button_relate.png"});
    ...
    // N E W    T E A C H E R
    // Add a custom teacher icon
    Gtk::Image* teacher_image = Gtk::manage(new Gtk::Image{"button_teacher.png"});
    Gtk::ToolButton *teacher_button = Gtk::manage(new Gtk::ToolButton(*button_teacher));
    ...
    ///////////  C O M B O T E X t ------ B O X  ///////////////////////////////////////////////////////////////
    

    //  for relate student to Parent //
    vbox->pack_start(*relate_sp); 
   ...
   // Add a little space 
    ...
    Gtk::Image* course_image = Gtk::manage(new Gtk::Image{"newCourse.png"});
    ...
    Gtk::Image* section_image = Gtk::manage(new Gtk::Image{"newSection.png"});
    ...
    //   S E M E S T E R   
    Gtk::Image* semester_image = Gtk::manage(new Gtk::Image{"button_semester.png"});
    Gtk::ToolButton *semester_button = Gtk::manage(new Gtk::ToolButton(*semester_image));
    semester_button->set_tooltip_markup("Create a new student");
    semester_button->signal_clicked().connect([this] {this->on_new_semester_click();});
    toolbar->append(*semester_button);


    // S E T    T H E     G R A D E
    Gtk::Image* grade_image = Gtk::manage(new Gtk::Image{"grade_student.png"});
    Gtk::ToolButton *grade_button = Gtk::manage(new Gtk::ToolButton(*grade_image));
    grade_button->set_tooltip_markup("Create a new student");
    grade_button->signal_clicked().connect([this] {this->set_grade_click();});
...
void Mainwin::on_student_parent_click() {  // Relate student and parent
    ...
    std::ostringstream oss_parent;
    std::ostringstream oss_student;

    Gtk::Dialog dialog{"Relate Student to Parent", *this};
    Gtk::Grid grid;
    dialog.get_content_area()->add(grid);

    

    Gtk::ComboBoxText cbt_parent;
    grid.attach(cbt_parent,0,0,3,1);


    for (int i = 0 ; i < person->parents() ;i++)
    {
        oss_parent.str(""); //clear the string steam
        oss_parent << person->parent(i);
        cbt_relate_ps.append(oss_parent());   
    }
    cbt_parent.set_active(person->parent()-1);
    dialog.get_content_area()->add(cbt_parent);


    Gtk::ComboBoxText cbt_student;
    grid.attach(cbt_student,0,1,3,1)
    for (in ti = 0; i < person->students() ; i++)
    {
        oss_student.str(""); //clear the string steam
        oss_student << person->student(i);
        cbt_relate_ps.append(oss_student());   
    }

    cbt_student.set_active(person->student()-1);
    dialog.get_content_area()->add(cbt_student;)
...
void Mainwin::on_about_click() {
    ...
    dialog.set_transient_for(*this);
    auto logo = Gdk::Pixbuf::create_from_file("About_smart_logo.png");
    dialog.set_logo(logo);
    dialog.set_version("Version " + VERSION);
    dialog.set_copyright("Copyright " + COPYRIGHT);
    dialog.set_license_type(Gtk::License::LICENSE_GPL_3_0);
    std::vector< Glib::ustring > authors = {"Saurav Kumar Sah"};
    dialog.set_authors(authors);
    std::vector< Glib::ustring > artists = {
        "Hassan, M. H. (2017, June 22). Computer Icon Education Studying University Alumni [Illustration]. Https://Pixabay.Com/Users/Mohamed_hassan-5229782/. https://pixabay.com/illustrations/computer-icon-education-studying-2429310/    ", 
        "Saputra, F. E. S. (2020, August 14). Pair Parent And Child Icon Avatar People User [Illustration]. Https://Pixabay.Com/Users/Febrianes86-5873902/. https://pixabay.com/vectors/pair-parent-and-child-icon-avatar-5484832/    ", 
        "OpenClipart-Vectors. (2016, April 1). Boy Daughter Family Father Favorites 4 Four Girl [Icon]. Https://Pixabay.Com/Vectors/Boy-Daughter-Family-Father-1300621/. https://pixabay.com/vectors/boy-daughter-family-father-1300621/    "
        "Smartschool. (2019, February 3). Logo of Smartschool [Illustration]. Wikimedia Commons. https://commons.wikimedia.org/wiki/File:Smartschool_logo.svg",
        "Ahkâm. (n.d.). Drawing Courses Icon PNG Transparent Background [Image]. Free Icons Png. https://www.freeiconspng.com/img/15341",
        "Gichia, W. G. (n.d.). Class Room Icon Png [Image]. PNG Item. https://www.pngitem.com/middle/imiThm_classroom-icon-png-clipart-png-download-class-icon/",
        "School Icon. (n.d.). [Image]. Seek Png. https://www.seekpng.com/ipng/u2q8i1w7q8i1i1i1_school-icon-logo-facilities/",
        "Open File Folder Icon – Free Download, PNG and Vector.” Icons8, icons8.com/icon/C8gfz6xPL7TZ/open-file-folder. ",
        "CHESNE, A. C. (2018, June 10). Guardar save icon [Image]. Icons-for-Free.Com. https://icons-for-free.com/guardar+save+icon-1320191245245380651/",
        "Save as Icon. (n.d.). [Image]. IconArchive. https://iconarchive.com/show/flatastic-9-icons-by-custom-icon-design/Save-as-icon.html",
    };
...
void Mainwin::on_new_teacher_click()
{
    try {
        ...
        parents.push_back(new Teacher{name.get_text(), email.get_text()});
    } catch(std::exception& e) {
        error("Invalid input", e);
    }
...
void Mainwin::set_grade_click()
{
    GtK::Dialog dialog{"Assign Grade",*this};
    Gtk::Grid grid;

    dilog.get_content_area()->add(grid);


    Gtk::ComboBoxText cbt_transcript;
    grid.attach(cbt_transcript,0,0,8,1);


    dialog.add_button("CANCEL",0);
    dialog.add_button("A",Grade::A);
    dialog.add_button("B",Grade::B);
    dialog.add_button("C",Grade::C);
    dialog.add_button("D",Grade::D);
    dialog.add_button("F",Grade::F);
    dialog.add_button("I",Grade::Incomplete);
    dialog.add_button("X", Grade::In Progress);
